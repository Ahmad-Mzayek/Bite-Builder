...
export const toggleLoadingAnimation = (overlay, loadingAnimationSpinner, isLoading) => {
  ...
  overlay.classList.replace("z-20", "z-30");
  toggleVisibility(loadingAnimationSpinner, isLoading);
};

export const addClosePopupSvgListeners = (svgs, overlay, pagePopups) => {
  svgs.forEach((svg) => {
    svg.addEventListener("click", () => {
      ...
      if (isAnyPopupVisible(pagePopups)) {
        overlay.classList.replace("z-30", "z-20");
      } else toggleVisibility(overlay, false);
    });
  });
};

export const addPasswordIconEventListeners = (passwordIcons) => {
